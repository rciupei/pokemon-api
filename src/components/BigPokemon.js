...
export default class Pokemon extends Component {
  ...
  async componentDidMount() {
    try {
      const res = await pokeApi.get(
        `/pokemon/${this.props.match.params.searchTerm}`
      );
      this.setState({
        data: res.data,
        imageUrl: res.data.sprites.other.dream_world.front_default,
        isLoading: false,
        error: false,
      });
    } catch (error) {
  ...
  render() {
    ...
    if (this.state.error) {
      return <h3>Error: Unable to find pokemon</h3>;
    }
    let { name } = this.state.data.species;

    return (
      <div className="container h-100 mt-2 d-flex justify-content-center flex-column ">
            ...
            <div className="col-md-4">
              ...
              >
                ...
                <div className="carousel-inner">
                  ...
                  <div className="carousel-item">
                    <img
                      src={this.state.data.sprites.back_default}
                      className="d-block w-100"
                      alt="back_default"
                    />
                  </div>
